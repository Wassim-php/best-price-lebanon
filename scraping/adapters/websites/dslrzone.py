...
class DslrZoneAdapter(BaseAdapter):
    """Adapter for dslr-zone.com WooCommerce search results."""

    source_name = "dslrzone"
    base_url = "https://www.dslr-zone.com"

    FREE_SHIPPING_THRESHOLD = 350.0
    ...
    def search(self, query: str, limit: int = 10, page: int = 1) -> List[OfferData]:
        api_offers = self._search_store_api(query, limit=limit, page=page)
        if api_offers:
            return api_offers

        if page and page > 1:
            search_url = f"{self.base_url}/a-instock/page/{page}/?s={quote_plus(query)}&post_type=product"
        else:
            search_url = f"{self.base_url}/a-instock/?s={quote_plus(query)}&post_type=product"

        try:
            logger.info("Searching DSLR Zone: %s", search_url)
            ...
            logger.error("Error searching DSLR Zone: %s", e)
            return []

    def get_detailed_pricing(self, product_url: str, location: str = "outside beirut") -> Dict[str, Any]:
        """
        Return final delivered price for DSLR Zone.

        DSLR Zone has no taxes. Orders of $350 or more qualify for free
        shipping; orders below $350 have a flat $5 shipping fee.
        """
        try:
            ...
            shipping_fee = self._shipping_fee_for_price(item_price)
            tax_amount = self.TAX_AMOUNT
            ...
            return {
                ...
                "breakdown": {
                    ...
                    "note": (
                        "DSLR Zone has no taxes. Orders of $350 or more get free "
                        "shipping; orders below $350 have flat $5 shipping."
                    ),
                },
            }

        except Exception as e:
            logger.error("Error getting DSLR Zone pricing: %s", e)
            return {
                "item_price": 0.0,
                "shipping_fee": None,
                "tax_amount": self.TAX_AMOUNT,
    ...
    def _search_store_api(self, query: str, limit: int = 10, page: int = 1) -> List[OfferData]:
        ...
        try:
            logger.info("Searching DSLR Zone Store API: %s", api_url)
            ...
            logger.warning("Error searching DSLR Zone Store API: %s", e)
            return []
    ...
    def _get_store_api_product(self, product_url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("Error fetching DSLR Zone Store API product: %s", e)
            return None
    ...
    def _extract_display_price(self, price_el) -> Optional[float]:
        ...
        screen_reader = price_el.select_one(".screen-reader-text")
        if screen_reader:
            screen_reader.extract()
    ...
    def _shipping_fee_for_price(self, item_price: float) -> float:
        if item_price >= self.FREE_SHIPPING_THRESHOLD:
            return 0.0
